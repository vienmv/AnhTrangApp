...
class SellerUpdateOrderShip extends Component {
  constructor(props) {
    super(props)
    this.state = {
  ...
  async loadAllShipper() {
    ...
    const searchDTO = {
      ...
      searchCity: this.props.order.cityName,
      search: {
        value: ""
      }
    }

    api.authFetch("/api/seller/accounts", "post", searchDTO,
      response => {
        response.json()
          .then(responseJson => {
...
export default SellerUpdateOrderShip;
