...
class AddCustomerOfUserGroup extends Component {
  constructor(props) {
    super(props)
    this.state = {
  ...
  async add() {
    ...
    api.authFetch("/api/admin/user-group/add-user", "post", { userGroupId: this.props.userGroup.id, listUserId: [this.state.userId] },
      (data) => {
          ...
          <Autocomplete
            ...
            renderItem={({ item }) => (
              <TouchableOpacity onPress={() => {
                this.setState({ userName: item.name, hideResults: true, userId: item.id })
              }}>
...
export default AddCustomerOfUserGroup;
